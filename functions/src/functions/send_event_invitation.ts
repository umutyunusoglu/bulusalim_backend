...
export const sendEventInvitation = onCall(async (request) => {
  ...
  const { toID, toUsername, toprofileImageUrl, eventID, eventName } = data;

  if (!toID || !eventID || !toUsername) {
    throw new HttpsError(
      "invalid-argument",
      "Gerekli parametreler (toID, eventID, toUsername) eksik.",
    );
  }
  ...
  try {
    ...
    if (!eventDoc.exists) {
      throw new HttpsError(
        "not-found",
        "Davet edilmek istenen buluşma bulunamadı.",
      );
    }
    ...
    // 7. İşlem Başarılı
    console.log(
      `Davet başarıyla gönderildi: From ${fromID} To ${toID} for Event ${eventID}`,
    );
    return {
      success: true,
    ...
    throw new HttpsError(
      "internal",
      "Davet işlemi sırasında teknik bir hata oluştu.",
      error.message,
    );
  }
});
