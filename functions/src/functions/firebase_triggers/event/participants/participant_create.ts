...
export const handleParticipantCreate = onDocumentCreated(
  ...
  async (event) => {
    try {
      ...
      const [userData, eventDoc, followersSnapshot, participantsSnapshot] =
        await Promise.all([
      ...
      await Promise.all([
        notifyUsers(followerIds, followerPayload),
        notifyUsers(otherParticipants, participantPayload),
      ]);

      logger.info(
        `Bildirimler ${followerIds.length} takipçiye ve ${otherParticipants.length} katılımcıya iletildi.`,
      );
    } catch (error) {
