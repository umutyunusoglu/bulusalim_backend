...
export const handleFollowerCreate = onDocumentCreated("users/{userId}/followers/{followerId}", async (event) => {
  try {
    ...
    const currentUserData = (await db.collection("users").doc(currentUser).get()).data();

    const currentID = currentUserData?.userID;
    const currentUsername = currentUserData?.username;
    const currentProfileImageUrl = currentUserData?.profileImageUrl;

    db.collection("users").doc(followerID).collection("followees").doc(currentUser).set({
      "userID": currentID,
