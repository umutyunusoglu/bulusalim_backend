...
export const deleteAccount = onCall(async (request) => {
  ...
  try {
   ...
   const cleanupPromises = subcollectionsToClean.map(async (collectionName) => {
    ...
    const eventsCreatedQuery = await db.collection("events")
      .where("creator.userID", "==", myUserID)
      .get();

    const eventHandoverPromises = eventsCreatedQuery.docs.map(async (eventDoc) => {
      ...
      const otherParticipants = participantsSnapshot.docs.filter((doc) => doc.data().userID !== myUserID);

      if (otherParticipants.length > 0) {
        // 1. Rastgele bir varis seç
        const randomParticipantDoc = otherParticipants[Math.floor(Math.random() * otherParticipants.length)];
    ...
    const postDeletePromises = postsQuery.docs.map((doc) => db.recursiveDelete(doc.ref));
